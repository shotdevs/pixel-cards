...
var Classic = async (option) => {
  ...
  if (!option.backgroundImage)
    option.backgroundImage = require("path").join(__dirname, "../assets/musicard-bg.png");
...
var Pixel = async (option) => {
  ...
  try {
    const canvas = (0, import_canvas5.createCanvas)(1240, 520);
    const ctx = canvas.getContext("2d");
    ...
    const bg = await (0, import_canvas5.loadImage)(bgSvg);
    ctx.drawImage(bg, 0, 0);

    // pixel top band (step-like blocks)
    const pixelBandSvg = generateSvg(`<svg width="1240" height="200" viewBox="0 0 1240 200" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- pixel blocks -->
      ${Array.from({length: 60}).map((_,i)=>{const h = Math.floor(Math.random()*40)+10; return `<rect x="${i*20}" y="${150-h}" width="16" height="${h}" fill="#AA88FF" fill-opacity="0.22"/>`;}).join('')}
      </svg>`);
    const pixelBand = await (0, import_canvas5.loadImage)(pixelBandSvg);
