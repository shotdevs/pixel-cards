// src/index.ts
import { createCanvas as createCanvas5, loadImage as loadImage5, GlobalFonts as GlobalFonts5 } from "@napi-rs/canvas";
...
var UserCard = async (options) => {
  ...
  if (backgroundImage) {
    try {
      const bgImage = await loadImage3(backgroundImage);
      ctx.drawImage(bgImage, 0, 0, width, height);
    } catch (e) {
...
// src/welcome-card.ts
import { createCanvas as createCanvas4, loadImage as loadImage4, GlobalFonts as GlobalFonts4 } from "@napi-rs/canvas";
...
var drawGlowingAvatar = async (ctx, x, y, size, avatarUrl) => {
  ...
  ctx.clip();
  try {
    const avatar = await loadImage4(avatarUrl);
    ctx.drawImage(avatar, x, y, size, size);
  } catch (e) {
...
var Pixel = async (option) => {
  ...
  const width = 1200;
  const height = 675;
  const canvas = createCanvas5(width, height);
  const ctx = canvas.getContext("2d");
  ctx.imageSmoothingEnabled = false;
  try {
    const fontPath = path5.join(__dirname, "..", "fonts", "pixel.ttf");
    if (!GlobalFonts5.has("PixelFont")) {
      GlobalFonts5.registerFromPath(fontPath, "PixelFont");
    }
  } catch (e) {
    console.error("Font not found. Make sure 'pixel.ttf' is in the 'fonts' folder.");
  }
  if (options.backgroundImage) {
    try {
      const bgImage = await loadImage5(options.backgroundImage);
      ctx.drawImage(bgImage, 0, 0, width, height);
    } catch (e) {
  ...
  try {
    const thumbnail = await loadImage5(options.thumbnailImage);
    ctx.imageSmoothingEnabled = true;
    ctx.drawImage(thumbnail, thumbX, thumbY, thumbSize, thumbSize);
    ctx.imageSmoothingEnabled = false;
  } catch (e) {
